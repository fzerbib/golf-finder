#!/usr/bin/env ruby
# One-off script: pulls the full list of golf courses in France and Spain from
# OpenStreetMap (Overpass API), grouped by official region / comunidad autónoma.
# Output: data/osm_golfs.json (raw, deduped, name+location only — no price/slope,
# OSM does not carry that data).
require "net/http"
require "uri"
require "json"
...
def fetch_region(iso_code, country_label, attempt = 1)
  query = <<~QL
    [out:json][timeout:60];
  ...
  if res.code == "429" && attempt <= 4
    wait = 20 * attempt
    warn "  ! HTTP 429 for #{iso_code}, retrying in #{wait}s (attempt #{attempt})"
    sleep wait
    return fetch_region(iso_code, country_label, attempt + 1)
  end
...
begin
  ...
  puts "Written to data/osm_golfs.json"
end
